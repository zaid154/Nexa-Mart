...
export const getRefreshCookieOptions = () => {
  // In production the site (Vercel) and the API (Render) are different domains,
  // so the refresh cookie travels cross-site. A browser only sends such a cookie
  // when it is SameSite=None, and it only accepts SameSite=None over HTTPS —
  // which is exactly when COOKIE_SECURE is true. With "lax" the cookie was
  // stored but never sent back, so every session died at the 15-minute mark.
  // Locally (http://localhost) we stay on "lax", the correct value same-site.
